...
item_view_event_count = 0
item_buy_event_count = 0
for event in events:
    if event["eventType"] == "itemViewEvent":
        item_view_event_count += 1
    if event["eventType"] == "itemBuyEvent":
        item_buy_event_count += 1
print(f"Число событий с типом itemViewEvent: {item_view_event_count}\nЧисло событий с типом itemBuyEvent: {item_buy_event_count}")


def ifDuplicates(events):
    duplicates = False
    for event in events:
    ...
    if duplicates is True:
        ...
        return "Нет"
    # или return duplicates and "YES" or "NO"
print(f"События содержат дубли: {ifDuplicates(events)}")
...
print(f"Самым первым по времени случилось событие с timestamp: {min_} и sessionId: {sessionId_}")


item_price_sum = 0
for event in events:
    item_price_sum += event["item_price"]
print(f"Общая сумма всех продуктов, которые просматривали (каждое событие — один просмотр): {item_price_sum}")
...
print(f"Событие, которое является первым для сессии: {pageViewId_}")